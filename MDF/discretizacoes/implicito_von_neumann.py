...
class ImplicitoVonNeumann:
    """
    Classe para realizar discretização pelo método de Euler Implícito com condições de fronteira de von Neumann
    ...
    def set_dados(self, L0: float, Ln: float, T: float, nx: int, nt: int, a: float, cond="dist_norm", fronte="zeros",
        ...
        self.A = cria_matriz_ampliada_von_neumann(-self.dados["sigma"], 1 + 2 * self.dados["sigma"], -self.dados["sigma"],
                                                  -2 * self.dados["sigma"], self.dados["nx"])
    ...
    def calcula(self):
        ...
        *A matriz c tem nt-1 elementos por isso utilizei i-1 e não i
        ...
        for i in range(1, self.dados["nt"]):
            self.dados["u"][i] = np.linalg.solve(self.A, self.dados["u"][i - 1] + self.c[i - 1])
