...
class CrankNicolsonDirichlet:
    """
    Classe para realizar discretização pelo método de Crank-Nicolson
    ...
    def set_dados(self, L0: float, Ln: float, T: float, nx: int, nt: int, a: float, cond="dist_norm", fronte="zeros",
        ...
        self.A = cria_matriz_tridiagonal(-self.dados["sigma"], 2+2*self.dados["sigma"], -self.dados["sigma"],
                                         self.dados["nx"]-2)
        self.B = cria_matriz_tridiagonal(self.dados["sigma"], 2-2*self.dados["sigma"], self.dados["sigma"],
                                         self.dados["nx"]-2)
        self.c = cria_matriz_vetor_auxiliar(self.dados["sigma"], self.dados["fx"], self.dados["gx"], self.dados["nx"]-2,
                                            self.dados["nt"]-1)
        self.calcula()
    ...
    def calcula(self):
        ...
        *A matriz c tem nt-1 elementos por isso utilizei i-1 e não i
        ...
        for i in range(1, self.dados["nt"]):
            ...
            self.dados["u"][i][1:-1] = np.linalg.solve(self.A, np.dot(self.B, self.dados["u"][i-1][1:-1])+self.c[i-1])
