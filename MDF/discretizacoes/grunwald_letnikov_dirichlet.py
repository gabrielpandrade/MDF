...
from MDF.utils.cria_matriz_triangular_inferior import cria_matriz_triangular_inferior
from MDF.utils.cria_matriz_vetor_auxiliar import cria_matriz_vetor_auxiliar
from MDF.utils.cria_vetor_coeficientes_g import cria_vetor_coeficientes_g


class GrunwaldLetnikovDirichlet:
    """
    Classe para realizar discretização pelo método de Grunwald-Letnikov
    ...
    g : numpy.ndarray
        Vetor cotendo todos os coefientes G para discretização de Grunwald-Letnikov
    ...
    def __init__(self, L0: float, Ln: float, T: float, nx: int, nt: int, a: float, alfa: float):
        """
        ...
        self.c = None
        self.g = None
        self.set_dados(L0, Ln, T, nx, nt, a, alfa)

    def set_dados(self, L0: float, Ln: float, T: float, nx: int, nt: int, a: float, alfa: float, cond="dist_norm", fronte="zeros",
                  frontd="zeros"):
        """
        ...
        self.dados["alfa"] = alfa
        self.set_condicao_inicial(cond)
        self.set_fronteira_esquerda(fronte)
        self.set_fronteira_direita(frontd)
        self.g = cria_vetor_coeficientes_g(self.dados["alfa"], self.dados["nx"])
        self.A = cria_matriz_triangular_inferior(self.g, self.dados["nx"]-2, self.dados["sigma"])
    ...
    def plota_2D(self, plots: int):
        ...
        plot_2D("Evolução de Grunwald-Letnikov",
                self.dados["nt"],
    ...
    def plota_3D(self):
        ...
        """
        plot_3D("Evolução de Grunwald-Letnikov",
                self.dados["x"],
                self.dados["t"],
                self.dados["u"]
                )