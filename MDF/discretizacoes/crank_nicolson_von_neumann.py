...
from MDF.utils.cria_matriz_ampliada_von_neumann import cria_matriz_ampliada_von_neumann
from MDF.utils.cria_matriz_vetor_auxiliar_von_neumann import cria_matriz_vetor_auxiliar_von_neumann


class CrankNicolsonVonNeumann:
    """
    Classe para realizar discretização pelo método de Crank-Nicolson com froteira de von Neumann
    ...
    def set_dados(self, L0: float, Ln: float, T: float, nx: int, nt: int, a: float, cond="dist_norm", fronte="zeros",
        ...
        self.A = cria_matriz_ampliada_von_neumann(-self.dados["sigma"], 2 + 2 * self.dados["sigma"],
                                                  -self.dados["sigma"], -2*self.dados["sigma"], self.dados["nx"])
        self.B = cria_matriz_ampliada_von_neumann(self.dados["sigma"], 2 - 2 * self.dados["sigma"],
                                                  self.dados["sigma"], 2*self.dados["sigma"], self.dados["nx"])
        self.c = cria_matriz_vetor_auxiliar_von_neumann(self.dados["dx"], self.dados["fx"], self.dados["gx"],
                                                        self.dados["nx"], self.dados["nt"]-1)
        self.calcula()
    ...
    def calcula(self):
        ...
        *A matriz c tem nt-1 elementos por isso utilizei i-1 e não i
        ...
        for i in range(1, self.dados["nt"]):
            self.dados["u"][i] = np.linalg.solve(self.A, np.dot(self.B, self.dados["u"][i-1])+self.c[i-1])
    ...
    def plota_2D(self, plots: int):
        ...
        """
        plot_2D("Evolução do Método de Crank-Nicolson",
                self.dados["nt"],
    ...
    def plota_3D(self):
        ...
        """
        plot_3D("Evolução do Método de Crank-Nicolson",
                self.dados["x"],
                self.dados["t"],
                self.dados["u"]
                )